...
def main():
    ...
    parser.add_argument(
        "--input_dir", 
        type=str, 
        default="../../data/raw/talkbank_dementia_split",
        help="Input directory containing audio files (default: '../data/raw_data/train_audios/')."
    )
    parser.add_argument(
        "--csv_file", 
        type=str, 
        default="../../data/interim/all_audios_original_labels.csv",
    ...
    parser.add_argument(
        "--output_dir", 
        type=str, 
        default="../../data/processed/post_data/",
        help="Output directory to save processed audio files (default: '../data/post_data/')."
    )    
    parser.add_argument(
        "--dataset_type", 
        type=str, 
        default="talkbank-parallel", 
        help="Type of dataset to process, talkbank or talkbank-parallel."
    )
    logger = setup_logger()
    ...
    output_dir = os.path.join(args.output_dir,'train_audios') if is_train else os.path.join(args.output_dir,'test_audios')
    if use_segment:
        output_dir = output_dir+'_segments'
    dataset_type = args.dataset_type

    # safeguard to dataset type option
    os.makedirs(output_dir, exist_ok=True)
    if dataset_type == "talkbank-parallel": 
